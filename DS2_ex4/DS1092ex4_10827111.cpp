...
class AdjList {
	...
	void BubbleSort3(vector<sT2> &answer) { // 泡沫排序(被呼叫用) // 以ID做排序 
		...
		for (i = answer.size() - 1; i > 0; i--) {
			for (j = 0; j <= i - 1; j++) {
				if ( atoi(answer[j].getID) > atoi(answer[j + 1].getID) ) {
					swap(answer[j], answer[j + 1]);
				} // if 
	...
	void BFS()
	{
		...
		while ( i < head.size()  ) {
			...
			while ( queue.size() > 0 ) { // queue裡面有東西 
				...
				int k = 0;
				while ( k < head.size() ) {
					if ( strcmp(head[k].putID, queue[0].getID) == 0 ) { // 尋找下一個PutID 一樣就是找到了 
						j = 0 ;
	...
	void DFS( float num )
	{
		...
		while ( i < head.size()  ) {
			...
			int j = 0 ;
			while ( j < head[i].samePut.size() ) { // 將後面的收訊者放進queue 
				if( head[i].samePut[j].weight >= num )
					queue.push_back(head[i].samePut[j]) ;	
				j++ ;
			} // while
			...
			while ( queue.size() > 0 ) { // queue裡面有東西 
				...
				strcpy(temp2.getID, queue[0].getID); // 將收訊者ID放進head3
